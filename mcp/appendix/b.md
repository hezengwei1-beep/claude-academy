
# 附录 B：官方资源与社区索引

## 官方网站与文档

| 内容 | URL |
|------|-----|
| MCP 官网 | https://modelcontextprotocol.io |
| 介绍 | https://modelcontextprotocol.io/introduction |
| 架构概念 | https://modelcontextprotocol.io/docs/learn/architecture |
| 构建 Server | https://modelcontextprotocol.io/docs/develop/build-server |
| 构建 Client | https://modelcontextprotocol.io/docs/develop/build-client |
| 调试指南 | https://modelcontextprotocol.io/docs/tools/debugging |
| Inspector | https://modelcontextprotocol.io/docs/tools/inspector |
| 安全最佳实践 | https://modelcontextprotocol.io/docs/tutorials/security/security_best_practices |
| 客户端列表 | https://modelcontextprotocol.io/clients |
| 示例 Server | https://modelcontextprotocol.io/examples |
| 路线图 | https://modelcontextprotocol.io/development/roadmap |
| MCP Registry | https://registry.modelcontextprotocol.io |

## 协议规范

| 内容 | URL |
|------|-----|
| 当前规范 | https://modelcontextprotocol.io/specification/2025-11-25 |
| Tools 规范 | https://modelcontextprotocol.io/specification/2025-11-25/server/tools |
| Resources 规范 | https://modelcontextprotocol.io/specification/2025-11-25/server/resources |
| Prompts 规范 | https://modelcontextprotocol.io/specification/2025-11-25/server/prompts |
| Transports 规范 | https://modelcontextprotocol.io/specification/2025-11-25/basic/transports |
| Authorization 规范 | https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization |

## GitHub 仓库

| 仓库 | URL |
|------|-----|
| 组织首页 | https://github.com/modelcontextprotocol |
| 协议规范 | https://github.com/modelcontextprotocol/specification |
| 官方 Servers | https://github.com/modelcontextprotocol/servers |
| TypeScript SDK | https://github.com/modelcontextprotocol/typescript-sdk |
| Python SDK | https://github.com/modelcontextprotocol/python-sdk |
| Go SDK | https://github.com/modelcontextprotocol/go-sdk |
| Inspector | https://github.com/modelcontextprotocol/inspector |
| Registry | https://github.com/modelcontextprotocol/registry |

## SDK 文档

| SDK | 文档 URL |
|-----|---------|
| TypeScript | https://ts.sdk.modelcontextprotocol.io |
| Python | https://py.sdk.modelcontextprotocol.io |
| C# | https://csharp.sdk.modelcontextprotocol.io |
| Go | https://go.sdk.modelcontextprotocol.io |
| Java | https://java.sdk.modelcontextprotocol.io |
| Rust | https://rust.sdk.modelcontextprotocol.io |
| Kotlin | https://kotlin.sdk.modelcontextprotocol.io |

## 社区资源

| 资源 | URL |
|------|-----|
| awesome-mcp-servers | https://github.com/punkpeye/awesome-mcp-servers |
| GitHub Discussions | https://github.com/orgs/modelcontextprotocol/discussions |
| Discord 社区 | https://discord.gg/6CSzBmMkjX |
| Anthropic MCP 公告 | https://www.anthropic.com/news/model-context-protocol |
